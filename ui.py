...
RAG_DEBUG_BASE_URL = _clean_base(os.getenv("RAG_DEBUG_BASE_URL")) or RAG_PIPELINE_URL
ENABLE_BACKEND_EMBED_TARGETS = (
    os.getenv("ENABLE_BACKEND_EMBED_TARGETS", "true").strip().lower() not in {"0", "false", "no"}
)
...
def _build_embed_targets() -> List[Tuple[str, str]]:
    ...
    candidate_specs: List[Tuple[str, Optional[str], List[str]]] = []
    if ENABLE_BACKEND_EMBED_TARGETS:
        candidate_specs.append(("backend", BACKEND_BASE_URL, BACKEND_EMBED_ASYNC_PATHS))
    ...
    for label, base, paths in candidate_specs:
        if not base:
            continue
        for path in paths:
            url = f"{base}{path}"
...
def _build_status_targets(job_id: str) -> List[Tuple[str, str]]:
    ...
    candidate_specs: List[Tuple[str, Optional[str], List[str]]] = []
    if ENABLE_BACKEND_EMBED_TARGETS:
        candidate_specs.append(("backend", BACKEND_BASE_URL, BACKEND_STATUS_PATHS))
    candidate_specs.extend(
        [
...
def _build_rag_debug_targets(path: str) -> List[Tuple[str, str]]:
    ...
    seen: set[str] = set()
    for label, base in [
        ("rag", RAG_DEBUG_BASE_URL),
        ("direct", RAG_DIRECT_URL),
        ("backend", BACKEND_BASE_URL if ENABLE_BACKEND_EMBED_TARGETS else None),
    ]:
        if not base:
            continue
        url = f"{base}{path}"
